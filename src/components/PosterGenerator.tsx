...
export const PosterGenerator: React.FC<PosterGeneratorProps> = ({ event, language, user, onGoogleLogin }) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      setPrompt(`Festival greeting poster for "${title}". Description: ${desc}`);
    } else {
      setPrompt("A vibrant cultural celebration poster with marigold flowers, oil lamps, and festive decorations.");
    }
  }, [event, language]);
